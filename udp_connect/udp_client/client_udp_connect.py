#!/usr/bin/env python3
"""
client_udp_connect.py - Authored by Daniel Theodore Seibert (https://github.com/whistlingelk/)
Copyright (c) 2025 Daniel Theodore Seibert released under the MIT License.

Short description:
  Provides a WebSocket-based client on the CLIENT that connects to an encrypted frame stream from the SERVER,
  iterating through a list of candidate IP addresses until a successful connection is established.
...
How it works:
  ...
  • Upon connection, receives encrypted frames and decrypts them using decrypt_data from client_encryption,
    decodes with OpenCV (converting from BGR to RGB), and displays the video stream.
  • Logs connection attempts and handles graceful shutdown.
  • Does not output detailed failure reasons.
...
Dependencies:
  ...
  • client_encryption – Provides decrypt_data for decryption.
...
Functions:
  ...
  • receive_and_display() -> None:
      Iterates through candidate IPs; attempts WebSocket connection, decrypts, and displays frames.
  • main() -> None:
...
import numpy as np
import websockets
from client_encryption import decrypt_data
        ...
        try:
            async with websockets.connect(ws_url) as websocket:
                ...
                while True:
                    ...
                    cv2.imshow("WebSocket Encrypted Feed", img)
